...
function TableSkeleton({ columns }: { columns: number }) {
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card">
      <div className="grid gap-4 border-b border-border bg-muted/60 px-4 py-3" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
      ...
      <div className="space-y-3 px-4 py-4">
        {Array.from({ length: 4 }).map((_, row) => (
          <div key={row} className="grid gap-4 py-2" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
